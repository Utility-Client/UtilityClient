...
public class TextureCompass extends TextureAtlasSprite
{
     ...
     */
    public void updateCompass(World worldIn, double px, double pz, double pr, boolean p_94241_8_, boolean p_94241_9_)
    {
        if (!this.framesTextureData.isEmpty())
        {
            ...
            if (worldIn != null && !p_94241_8_)
            {
                ...
                double d1 = (double)blockpos.getX() - px;
                double d2 = (double)blockpos.getZ() - pz;
                pr = pr % 360.0D;
                d0 = -((pr - 90.0D) * Math.PI / 180.0D - Math.atan2(d2, d1));
                if (!worldIn.provider.isSurfaceWorld()) d0 = Math.random() * Math.PI * 2.0D;
            }

            if (p_94241_9_) this.currentAngle = d0;
            else
            {
                double d3;
                for (d3 = d0 - this.currentAngle; d3 < -Math.PI; d3 += (Math.PI * 2D)) {}
                while (d3 >= Math.PI) d3 -= (Math.PI * 2D);
